...
function Payment() {
    ...
    this.requestUrl = 'https://openapi.alipay.com/gateway.do';
    // eslint-disable-next-line prefer-rest-params
...
Payment.mix = function () {
    ...
    case 2:
        {
            const key = arguments[0].toString();
...
Payment.mix('sign', function (param) {
    const sign = util.sign(param, this.options.app_private_key, this.options.charset, this.options.sign_type);

    return sign;
});

Payment.mix('verify', function (param) {
    const result = util.verify(param, this.options.alipay_public_key, this.options.charset, this.options.sign_type);
...
Payment.mix('buildSignOrderParam', function (opts) {
    ...
    let str = util.buildQueryString(param, true);
...
Payment.mix('notify', (callback) => function (req, res, next) {
    const _this = this;
    ...
    util.pipe(req, (err, data) => {
        console.log(err);
        console.log('notify:', data);
        const xml = data.toString('utf8');
        util.parseXML(xml, (e, msg) => {
            if (e) {
                console.error(e);
            }
...
Payment.mix('buildFundAccountQuery', function (userId) {
    ...
    param.timestamp = util.formatDate(Date.now());
    // param.nonce_str = opts.nonce_str || util.generateNonceString();
    param.biz_content = {
